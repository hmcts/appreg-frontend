Feature: Application list entries result selected help details
  ...
  Background: Create application list entry
    ...
    When User Makes POST API Request To "/application-lists" With Body:
      ...
      | todayiso | 10:20 | OPEN   | Result selected help details list {SCENARIO_ID} | LCCC065           |
  ...
  Scenario: Expand and collapse list result selected help details
    ...
    When User Searches Application List With:
      ...
      | today |      | Result selected help details list {SCENARIO_ID} | LCCC065     |       |        |                |     |           |
    When User Clicks "Select" Then "Open" From Menu In Row Of Table "Lists" With:
      | Date         | Time  | Location                          | Description                                | Entries | Status |
      | todaydisplay | 10:20 | Leeds Combined Court Centre Set 7 | Result selected help details list {SCENARIO_ID} | 1       | OPEN   |
    When User Checks The Checkbox In Row Of Table "Entries" With:
      | Sequence number | Account number   | Applicant             | Respondent           | Postcode | Title                      | Fee | Resulted |
      | 1               | ACC-RES-{RANDOM} | Henry Taylor {SCENARIO_ID} | Emily Clark {SCENARIO_ID} | BS15 5AA | Condemnation of Unfit Food | Yes |          |
