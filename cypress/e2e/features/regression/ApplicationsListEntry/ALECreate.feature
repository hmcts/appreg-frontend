Feature: Applications List Entry Create
  ...
  Scenario Outline: Create an ALE where Applicant = Person and Respondent = Person, using an Application Code with Fee Required = Y and Respondent Required = Y
    ...
    Then User Should See Table Has Sortable Headers "Fee Status, Status Date, Payment Ref" In The Accordion "Civil fee"
    # Then User Clicks "Change" Link In Row Of Table "" With In The Accordion "Civil fee":
    ...
    # Then User Searches The Row In The Table "Fee Details" With Values:
    #   | Fee Status | Status Date   | Payment Ref                         |
    ...
    Examples:
      | User  | TableName | SearchDate | DisplayDate  | Time  | Court                             | Description                             | Entries | Status | SelectButtonText | ButtonName | ApplicationCode | ApplicationTitle           | WordingText                                      | placeholder                  | WordingValue        | PaymentReferenceUndertaken | UpdatedPaymentReferenceUndertaken | PaymentReferencePaid | CaseReference | AccountReference | OffsiteFeeString                                         | OffsiteFeeCode | OffsiteFeeValue             | TotalFeeAmount            | FeeReference         | FeeAmount       |
      | user1 | Lists     | today      | todaydisplay | 10:20 | Leeds Combined Court Centre Set 7 | Applications to review at Test_{RANDOM} | 0       | OPEN   | Select           | Open       | MX99006         | Condemnation of Unfit Food | Application for the condemnation of food, namely | Enter a Describe Seized Food | Test Sample Wording | PAY-12345                  | New PAY-12345                     | Paid PAY-12345       | case12345     | account12345     | Selecting this will apply the off site fee to the entry. | CO1.1          | Off Site Fee Amount: £30.00 | Total Fee Amount: £284.00 | Fee Reference: CO8.1 | Amount: £284.00 |
