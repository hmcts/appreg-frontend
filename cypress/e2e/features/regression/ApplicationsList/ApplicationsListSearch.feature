Feature: Applications List Search
  ...
  @regression @applicationsList @ARCPOC-1368
  Scenario: Verify entries table sorting after opening an applications list
    Given User Is On The Portal Page
    When User Signs In With Microsoft SSO As "user1"
    # Search for open lists, then open the list that has entries
    When User Searches Application List With:
      | Date       | Time  | List description                | CourtSearch | Court | Select list status | Other location description | Criminal justice area | CJASearch |
      |            |       |                                  |             |       | Open               |                            |                       |           |
    When User Clicks "Select" Then "Open" From Menu In Row Of Table "Lists" With:
      | Date        | Time  | Location   | Description                     | Entries | Status |
      | 12 Oct 4044 | 12:15 | Manchester | Delta Echo Lima Echo Tango Echo | 4       | OPEN   |
    ...
    # Verify sorting on each Entries table column
