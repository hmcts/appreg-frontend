...
Then('User Verify Response Status Code Should Be {string}', (expectedStatusCode: string) => {
...
Then('User Verify Response Body Should Have:', (dataTable: DataTable) => {
  cy.get('@lastApiResponse').then((response) => {
    ...
    dataTable.raw().forEach(([header, value]) => {
      ...
      cy.log(`Assert: ${header} | Expected: ${JSON.stringify(expected)} | Actual: ${JSON.stringify(actual)}`);
        if (Array.isArray(expected)) {
          if (Array.isArray(actual)) {
            ...
            throw new Error(`Expected an array for property ${header}, but got ${typeof actual}`);
          }
