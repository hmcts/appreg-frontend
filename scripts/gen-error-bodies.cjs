...
const MAP = {
  'bad-request-error.yaml': {
    out: 'bad-request.json',
    status: 400,
    title: 'Bad Request',
  },
  'unauthorized-error.yaml': {
    out: 'unauthorized.json',
    status: 401,
    title: 'Unauthorized',
  },
  'forbidden-error.yaml': {
    out: 'forbidden.json',
    status: 403,
    title: 'Forbidden',
  },
  'not-found-error.yaml': {
    out: 'not-found.json',
    status: 404,
    title: 'Not Found',
  },
  'not-acceptable-error.yaml': {
    out: 'not-acceptable.json',
    status: 406,
  ...
  'payload-too-large-error.yaml': {
    out: 'payload-too-large.json',
    status: 413,
    title: 'Payload Too Large',
  },
  'unsupported-media-type-error.yaml': {
    out: 'unsupported-media-type.json',
    status: 415,
    title: 'Unsupported Media Type',
  },
  'internal-server-error.yaml': {
    out: 'internal-server-error.json',
    status: 500,
...
(async () => {
  ...
  for (const f of files) {
    ...
    const title =
      (y.description && String(y.description).trim()) || MAP[f].title;
