...
prompt = f"""You are the read-only planning phase of an automated bug-fixing workflow.

Analyse the Jira ticket and this Angular/Node frontend repository, then return a decision-complete implementation plan using only the supplied JSON schema.
...
- Explicitly compare a focused fix with any broader component, shared-service, validation, design-system, accessibility, API-contract, or cross-system correction that the evidence suggests.
...
- Follow the repository's Angular, TypeScript, HMCTS design-system, Prettier, ESLint, Stylelint, Jest, and Cypress patterns when identifying the intended change and tests.
