...
describe('OpenAPI client smoke (no backend)', () => {
  let http: HttpTestingController; let api: ApplicationCodesApi;
  ...
  it('GET /application-codes', () => {
    ...
    const req = http.expectOne(r => r.method === 'GET' && r.url.endsWith('/application-codes'));
    req.flush([]);
  });

  it('GET /application-codes/{code}?date=…', () => {
    api.getApplicationCodeByCodeAndDate({ code: 'AD99005', date: '2017-07-25T00:00:00Z' }).subscribe();
